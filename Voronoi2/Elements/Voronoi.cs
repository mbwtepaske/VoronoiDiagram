    ...
    private void Generate()
    {
      ...
      while (true)
      {
        ...
        if (nextSite != null && (IsEmptyPriorityQueue() || nextSite.Point.Y < newIntersection.Y || IsNearlyEqual(nextSite.Point.Y, newIntersection.Y) && nextSite.Point.X < newIntersection.X))
        {
          ...
          var rightBoundaryHalfEdge = leftBoundaryHalfEdge.Right;

          // if this halfedge has no edge, bot = bottom site (whatever that is)
          var bottomSite = RightRegion(leftBoundaryHalfEdge);
          ...
          var currentSite = Intersect(leftBoundaryHalfEdge, bisector);
          ...
          currentSite = Intersect(bisector, rightBoundaryHalfEdge);
        ...
        else if (!IsEmptyPriorityQueue()) // intersection is smallest - this is a vector event
        {
          ...
          var bottomSite = LeftRegion(leftBoundaryHalfEdge);
          ...
          var currentSite = Intersect(leftBoundaryHalfEdge.Left, bisector);
          ...
          currentSite = Intersect(bisector, rightBoundaryHalfEdge.Right);
      ...
      {
        ClipEdge(leftBoundaryHalfEdge.EdgeListEdge);
      }
    }

    public IEnumerable GenerateStep(IReadOnlyList<Point> points
      , double borderMinimumX
      , double borderMaximumX
      , double borderMinimumY
      , double borderMaximumY
